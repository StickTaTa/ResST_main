...
from sklearn.decomposition import IncrementalPCA, NMF, TruncatedSVD, FactorAnalysis, LatentDirichletAllocation, \
    sparse_encode
...
def cal_weight_matrix(
    ...
    if platform in ["Visium", "ST"]:
        ...
        # gene_counts = adata.X.copy().todense()
...
def enhance_gene_data(
        adata,
        adjacent_weight=0.2,
):
    adjacent_gene_matrix = adata.obsm["adjacent_data"].astype(float)
    if isinstance(adata.X, np.ndarray):
        enhance_gene_matrix = adata.X + adjacent_weight * adjacent_gene_matrix
    elif isinstance(adata.X, csr_matrix):
        enhance_gene_matrix = adata.X.toarray() + adjacent_weight * adjacent_gene_matrix
    adata.obsm["enhance_gene_data"] = enhance_gene_matrix
...
def enhance_adata(
    ...
    adata = find_adjacent_spot(adata,
                               ...
                               weights=weights)
    adata = enhance_gene_data(adata,
                              adjacent_weight=adjacent_weight, )
    return adata


def get_enhance_feature(adata, adjacent_weight=0.3, neighbour_k=4, weights="weights_matrix_all", spatial_k=30,
                        platform='Visium'):
    adata_enhance = enhance_adata(adata,
                                  adjacent_weight=adjacent_weight,
                                  neighbour_k=neighbour_k,
                                  platform=platform,
                                  weights=weights,
                                  spatial_k=spatial_k)
    return adata_enhance
